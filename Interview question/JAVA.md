...
---
### 방어적 복사에 대해서 설명해주세요.
방어적 복사란 원본과의 참조를 끊은 복사본을 만들어 사용하는 방식이며, 원본의 변경에 의한 예상치 못한 사이드 이펙트를 방지하여 안전한 코드를 만들 수 있는데 도움이 됩니다.

---
## 05 14 면접질문
### 데이터 베이스로 부터 데이터를 가져와 List에 담고 List를 가지고 빈번히 삽입, 삭제되는 로직이 이루어진다고 가정했을 때 ArrayList 와 LinkedList 중에 어떤 컬렉션을 사용할 것인가요?
만약 삽입 삭제가 담긴 데이터에 마지막 위치에 해당한다고 가정하였을때는 ArrayList를 사용해도 되지만 그게 아닌 중간에 삽입 삭제가 이루어진다면 LinkedList를 사용할 것 같습니다.
그 이유로는 ArrayList는 인덱스를 기반이고 LinkedList는 노드를 기반으로 구현되어 삽입 삭제가 빈번히 이루어진다면 해당하는 노드만 교체해주면 되기 떄문입니다.

### ArrayList에 담긴 데이터를 순회하면서 일치하는 값을 찾는 contains()메서드를 실행하였을때 어떤 식으로 바꾸시겠습니까?
HashSet을 사용할 것 같습니다. ArrayList의 contains()메서드는 순회하면서 찾기 때문에 시간복잡도가 O(n)에 해당하여 이를 HashSet으로 변환하여 할 것 같습니다.
HashSet에 경우에는 value 값을 hash로 치환하여 값을 조회하기 때문에 평균 시간복잡도 O(1)에 해당되기 떄문입니다.
